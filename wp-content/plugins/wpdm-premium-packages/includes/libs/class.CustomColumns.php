...
class WPDMPPCustomColumns {
    ...
    function ColumnsTH($defaults) {
        ...
        $otf['wpdmsaleamt'] = "<span title=".__('Total Sales','wpdm-premium-packages')." class='wpdm-th-icon ttip'><i class='fa fa-money'></i></span>";
    ...
    function ColumnsTD($column_name, $post_ID) {
        ...
        if ($column_name == 'wpdmsaleamt') {
            echo "<a title=".__('Total Sales, Click to Recalculate', 'wpdm-premium-packages')." class='ttip recal-sa' href='#' rel='{$post_ID}'>".wpdmpp_currency_sign().number_format((double)get_post_meta($post_ID, '__wpdm_sales_amount', true))."</a>";
        }
