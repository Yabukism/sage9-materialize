        ...
        <div class="col-md-12 wpdm-full-front">
                ...
                <div class="panel-body">
                    <div class="row">
                        <div class="col-md-4">
                            ...
                            <input type="text" size="16" class="form-control" id="base-price-field" name="file[base_price]"  value="<?php $base_price = get_post_meta($post->ID,'__wpdm_base_price',true); if(isset($base_price)) echo number_format((double)$base_price,2,'.','');?>">
                        </div>
                        <div class="col-md-4">
                            <label for="sales-price-field"><?php echo __('Sales Price','wpdm-premium-packages'); ?></label>
                            <input type="text" class="form-control" size="16" id="sales-price-field" name="file[sales_price]"  value="<?php $sales_price = get_post_meta($post->ID,'__wpdm_sales_price',true); echo number_format((double)$sales_price,2,'.','');?>">
                        </div>
                        <div class="col-md-4">
                            <label for="sales-price-expire-field"><?php echo __('Valid Until','wpdm-premium-packages'); ?></label>
                            <input type="text" class="form-control" size="16" id="sales-price-expire-field" name="file[sales_price_expire]"  value="<?php echo get_post_meta($post->ID,'__wpdm_sales_price_expire',true);?>">
                        ...
                        if(is_array($variation)){
                            foreach($variation as $key=>$vname){?>
                                <div id="variation_div_<?php echo $key;?>" class="panel panel-default">
                                    <div class="panel-heading">
                                        <?php _e('Group ID#','wpdm-premium-packages');  ?> <?php echo $key;?>
                                    ...
                                    <table class="table table-v" id="voption_table_<?php echo $key;?>">
                                        <tr><td colspan="3"><label><input style="margin: 0" type="checkbox" name="file[variation][<?php echo $key;?>][multiple]" placeholder="Multiple Select" <?php if(isset($vname['multiple'])) echo "checked='checked'"; ?> > &nbsp;<?php echo __('Multiple Select','wpdm-premium-packages'); ?></label></td></tr>
                                        <tr><td colspan="3"><input class="form-control" type="text" name="file[variation][<?php echo $key;?>][vname]" id="" placeholder="<?php _e('Group Name','wpdm-premium-packages');  ?>" title="<?php _e('Enter a Gig Name','wpdm-premium-packages');  ?>" value="<?php echo $vname['vname'];?>"></td></tr>
                                        <tr><th><?php _e('Gig Name','wpdm-premium-packages'); ?></th><th width="150px"><?php _e('Extra Cost','wpdm-premium-packages'); ?></th><th width="50px"><?php _e('Delete','wpdm-premium-packages'); ?></th></tr>
                                        <?php
                                        if($vname){
                                            foreach($vname as $optionkey=>$optionval){
                                                if($optionkey!="vname" && $optionkey != "multiple"){?>
                                                    <tr id="voption<?php echo $optionkey;?>"><td><input type="text" name="file[variation][<?php echo $key;?>][<?php echo $optionkey;?>][option_name]"  placeholder="Gig Name" class="form-control input-sm" value="<?php echo $optionval['option_name'];?>"></td><td><div class="input-group input-group-sm"><span class="input-group-addon"><i class="fa fa-plus-circle"></i></span><input style="max-width: 70px" min="0" name="file[variation][<?php echo $key;?>][<?php echo $optionkey;?>][option_price]" id="" size="5" class="form-control" type="number" placeholder="price" value="<?php echo $optionval['option_price'];?>"></div></td><td><i class="delet_voption fa fa-times-circle text-danger" rel="voption<?php echo $optionkey;?>" title="Delete this option" style="cursor:pointer"></i></td></tr>
                                                <?php
                                                }
                                            }
                                        }
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                        ...
                        if(is_array($free_downloads)){
                            ...
                            <div class="list-group-item" id="fdl<?php echo $id; ?>">
                                <div class="input-group">
                                    ...
                                    <span class="input-group-addon delete-freedl" data-id="#fdl<?php echo $id; ?>"><i class="fa fa-trash-o color-red"></i></span>
                                </div>
                            </div>
                    ...
                    <button type="button" id="addfreedls" class="btn btn-default btn-sm"><i class="fa fa-plus-circle color-green"></i> <?php _e('Add Free File(s)','wpdm-premium-packages'); ?></button>
                </div>
            </div>
            ...
            <script type="text/javascript">
                jQuery('#price_variation').click(function(){
                    if(jQuery('#price_variation').attr("checked")){
                        jQuery('#variation_heading').text("Gig Options");
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <label>
                        <input type="checkbox" id="licreq"  style="margin: 0 !important;" value="1" name="file[enable_license]" <?php if(get_post_meta($post->ID, "__wpdm_enable_license", true)==1) echo 'checked="checked"'; ?> > &nbsp;<?php _e('License Key Required','wpdm-premium-packages'); ?>
                    </label>
    ...
    jQuery(function ($) {
        ...
        $('body').on('click', '#addfreedls', function( event ){
            ...
            _file_frame.on( 'select', function() {
                ...
                $('#free-files').append("<div class='list-group-item'  id='"+ID+"'><div class='input-group'><input class='form-control' readonly=readonly name='file[free_downloads][]' value='"+file+"' /><span class='input-group-addon delete-freedl' data-id='#"+ID+"'><i class='fa fa-trash-o color-red'></i></span></div></div>");
