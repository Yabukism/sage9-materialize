...
<form method="post" action="edit.php?post_type=wpdmpro&page=wpdm-subscribers&task=delete&lockOption=google" id="posts-filter" class="panel-body">
    ...
    <table id="subtbl" cellspacing="0" class="table table-striped">
        ...
        <tr>
            ...
            <th style="width: 100px" class="manage-column column-password" id="author" scope="col" ><?php echo __( "Action" , "download-manager" ); ?></th>
        </tr>
        </thead>
        ...
        <?php foreach($res as $row) {
            ...
            <tr valign="top" class="author-self status-inherit" id="post-<?php echo $row['id']; ?>">
                ...
                <td class="author column-author">
                    <?php
                    if ($row['processed']==0) echo "<a href='".admin_url("?export_google_contacts=".$row['ID'])."' class='btn btn-xs btn-default'>Export Contacts</a>";
    ...
    <div class="well">
        <nobr>
            <input type="submit" class="btn btn-default btn-sm action submitdelete" id="doaction" value="<?php echo __( "Delete Selected" , "download-manager" ); ?>">
