...
class Package {
    ...
    function Prepare($ID = null, $template = null, $template_type = 'page')
    {
        ...
        if(!isset($post_vars['btnclass']))
            $post_vars['btnclass'] = '__wpdm_download_btn__';
        ...
        if (self::userDownloadLimitExceeded($post_vars['ID'])) {
            ...
            $post_vars['download_link_popup'] =
            $post_vars['download_link_extended'] =
            $post_vars['download_link'] = "<div class='alert alert-warning {$alert_size}' data-title='" . __( "DOWNLOAD ERROR:" , "download-manager" ) . "'>{$post_vars['link_label']}</div>";
        }
        ...
        if(isset($data['terms_lock']) && $data['terms_lock'] != 0 && (!function_exists('wpdmpp_effective_price') || wpdmpp_effective_price($post_vars['ID']) ==0) && $limit_over == 0){
            ...
            $data['terms_conditions'] = sanitize_textarea_field($data['terms_conditions']);
...
//wppmdd($post_vars);
    ...
    function prepareDownloadLink(&$package, $embed = 0, $extras = array())
    {
        ...
        if (wpdm_is_download_limit_exceed($package['ID'])) {
            ...
            $package['link_label'] = apply_filters("wpdm_download_limit_message",__( "Download Limit Exceeded" , "download-manager" ), $package);
        }
...
//                        $adata .= '<div class="modal fade" role="modal" id="pkg_' . $package['ID'] . "_" . $unqid . '"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><strong style="margin:0px;font-size:12pt">' . __('Download') . '</strong></div><div class="modal-body">' . $data . '</div><div class="modal-footer text-right"><button type="button" class="btn btn-default btn-sm" data-dismiss="modal">Close</button></div></div></div></div>';
    ...
    public static function fetchTemplate($template, $vars, $type = 'link')
    {
        ...
        if(is_user_logged_in() && !self::userCanAccess($vars['ID']) && get_option('_wpdm_hide_all', 0) == 1 ) return $type != 'page' ? "" : get_option('wpdm_permission_msg',__( "You are not allowed to download this item!" , "download-manager" ));
        ...
        if(isset($templates[$template]) && isset($templates[$template]['content'])) $template = $templates[$template]['content'];
            ...
            if(!strpos(strip_tags($template), "]")){
                ...
                else if (file_exists($themeltpldir . $template)) $template = file_get_contents($ltpldir . $template);
        ...
        $vars['fav_button_sm'] = self::favBtn($vars['ID'], array('size' => 'btn-sm', 'a2f_label' => "<i class='fa fa-star'></i> &nbsp; ". __( "Add to favourite" , "download-manager" ), 'rff_label' => "<i class='fa fa-star'></i> &nbsp; ". __( "Remove from favourite" , "download-manager" )));
    ...
    public static function favBtn($id, $atfb = array(), $count = true){
        if(empty($atfb))
            $atfb = array('size' => '', 'a2f_label' => "<i class='fa fa-star'></i> &nbsp;". __( "Add to favourite" , "download-manager" ), 'rff_label' => "<i class='fa fa-star'></i> &nbsp; ". __( "Remove from favourite" , "download-manager" ));
        ...
        extract($atfb);
        if($count)
            return "<div class='btn-group'><button type='button' data-alabel=\"{$atfb['a2f_label']}\" data-rlabel=\"{$atfb['rff_label']}\" data-package='{$id}' class='btn btn-wpdm-a2f {$btnclass} {$size}'>{$label}</button><button class='btn btn-default {$size}' disabled='disabled'>{$pfc}</button></div>";
        else
