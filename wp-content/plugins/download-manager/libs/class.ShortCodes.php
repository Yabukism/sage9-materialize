...
TBR;
    ...
    function searchResult( $params = array() ){
        ...
        $html = $this->Packages(array('items_per_page' => $items_per_page, 'template' => $template, 's' => $strm, 'paging' => true, 'toolbar' => 0,'cols'=>$cols, 'async' => $async));
        $html = "<div class='w3eden'><form style='margin-bottom: 20px'><div class='input-group input-group-lg'><div class='input-group-addon input-group-prepend no-radius'><i class='fas fa-search'></i></div><input type='text' name='search' value='".esc_attr(wpdm_query_var('search'))."' class='form-control input-lg no-radius' /></div></form>{$html}</div>";
