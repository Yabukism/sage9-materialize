...
//        ->Fetch('file-list.php');
        ...
        if (count($file['files']) > 0 || count($dfiles) > 0) {
            ...
            $allfiles = maybe_unserialize(get_post_meta($file['ID'], '__wpdm_files', true));
                //is_array($file['files'])?$file['files']:array();

            //$allfiles = array_merge($allfiles, $dfiles);
            $fhtml = "<audio autoplay controls preload='auto' style='width: 100%;display: none' id='audio-player-{$file['ID']}'></audio><table id='wpdm-filelist-{$file['ID']}' class='wpdm-filelist table table-hover'><thead><tr><th>".__( "File" , "download-manager" )."</th>{$pwdcol}{$dlcol}</tr></thead><tbody>";
            ...
            if (is_array($dfiles)) {
                ...
                foreach ($dfiles as $fileID => $sfile) {
                    ...
                    if ($idvdl == 1 && ($pwdlock || !$olock)) {
                        ...
                        $mp3 = end($mp3);
                        $mp3 = strtolower($mp3);
                        $play = in_array($mp3, array('mp3'))?"<a rel='nofollow' class='btn btn-success btn-sm wpdm-btn-play' href='#' data-player='audio-player-{$file['ID']}' data-song='" . $download_url . "&ind=" . $ind . "'><i style='margin-top:0px' class='fa fa-play'></i></a>":"";
        ...
        if (count($file['files']) > 0 || count($dfiles) > 0) {
            ...
            if (is_array($allfiles)) {
                ...
                foreach ($allfiles as $fileID => $sfile) {
                    ...
                    if($thumb)
                        $fhtml .= "<div class='panel-body text-center'><img class='file-thumb' src='{$thumb}' alt='{$fileTitle}' /></div><div class='panel-footer footer-info'>".wpdm_file_size($sfile)."</div><div class='panel-footer text-center'>";
                    else
                        $fhtml .= "<div class='panel-body text-center'><img class='file-ico' src='".WPDM_BASE_URL.'assets/file-type-icons/'.$ext.'.svg'."' alt='{$fileTitle}' /></div><div class='panel-footer footer-info text-center'>".wpdm_file_size($sfile)."</div><div class='panel-footer  text-center'>";
                    ...
                    if ($swl) {
                        ...
                        if ($filePass != '' && $pwdlock)
                            $fhtml .= "<span class='input-group-btn input-group-append'><button class='inddl btn btn-default btn-light btn-block' data-pid='{$file['ID']}' data-file='{$fileID}' data-pass='#pass_{$file['ID']}_{$ind}'><i class='fas fa-arrow-alt-circle-down'></i></button></span></div>"; //rel='" . $download_url . "&ind=" . $ind . "'
                        else {
                            $ind_download_link = "<a rel='nofollow' class='inddl btn btn-primary btn-sm' href='" . $download_url . "&ind=" . $ind . "'>".$button_label."</a>";
                            $ind_download_link = apply_filters("wpdm_single_file_download_link", $ind_download_link, $sfile, $download_url, $file);
                            $individual_file_actions = '';
            ...
            if (is_array($dfiles)) {
                ...
                foreach ($dfiles as $ind => $sfile) {
                    ...
                    if ($swl) {
                        ...
                        if ($fileinfo[$sfile]['password'] != '' && $pwdlock  && !is_dir($sfile))
                            $fhtml .= "<span class='input-group-btn input-group-append'><button class='inddl btn btn-primary btn-sm' file='{$sfile}' rel='" . $download_url . "&ind=" . $ind . "' pass='#pass_{$file['ID']}_{$ind}'><i class='fa fa-download'></i></button></span></div>";
