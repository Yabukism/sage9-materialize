...
class PackageLocks
{
            ...
            <div class="panel-body" id="wpdmdlp_<?php echo  $field_id; ?>">
                ...
                <form id="wpdmdlf_<?php echo $field_id; ?>" method=post action="<?php echo home_url('/'); ?>" style="margin-bottom:0px;">
                    ...
                    <div class="input-group input-group-lg">
                        ...
                        <span class="input-group-btn input-group-append"><input id="wpdm_submit_<?php echo $field_id; ?>" class="wpdm_submit btn btn-default" type="submit" value="<?php _e( "Submit" , "download-manager" ); ?>" /></span>
                    </div>
    ...
    public static  function askEmail($package)
    {
        ...
        if (isset($package['email_lock']) && $package['email_lock'] == '1') {
            ...
            $lock = 'locked';
            $unqid = uniqid();
            $btitle = isset($package['email_lock_title']) && $package['email_lock_title'] != ''? $package['email_lock_title']:__( "Subscribe To Download" , "download-manager" );
            $blabel = isset($package['button_label']) ? $package['button_label'] : __( "Download" , "download-manager" );
            $dlabel =  __( "Download Now" , "download-manager" );
            $eeml =  __( "Email Address" , "download-manager" );
            $intro = isset($package['email_intro']) ? "<p>" . $package['email_intro'] . "</p>" : '';
            $field_id = $unqid.'_'.$package['ID'];
            $data = '
                <div id="emsg_' . $unqid . $package['ID'] . '" class="emsg_' . $unqid . $package['ID'] . '" style="display:none;">'.__( "Processing..." , "download-manager" ).'</div>
  ...
  <div class="panel-body">
        ' . $intro . '
        <input type=hidden name="__wpdm_ID" value="' . $package['ID'] . '" />
        <input type=hidden name="dataType" value="json" />
        <input type=hidden name="execute" value="wpdm_getlink" />
        ...
        ';
            ...
            $data .= $html;
            $data .= '
        <label>'.apply_filters("send_download_link_to_label",__( "Send Download Link To:" , "download-manager" )).'</label>
        ...
        jQuery(".wpdmdlf_' . $field_id . '").submit(function(){
            ...
            jQuery(".emsg_' . $unqid . $package['ID'] . '").removeAttr("style").html("<div style=\'margin:10px 0\' data-title=\''.__( "PLEASE WAIT" , "download-manager" ).'\' class=\'alert alert-info\'><i class=\'fa fa-sync fa-spin\'></i> '.__( "Verifying Request..." , "download-manager" ).'</div>").show();
            ...
            success:function(res){
                jQuery(".wpdmdlf_' . $field_id . '").hide();
                jQuery(".emsg_' . $package['ID'] . '").html("<div style=\'margin:10px 0\' data-title=\''.__( "PLEASE WAIT" , "download-manager" ).'\' class=\'alert alert-info\'><i class=\'fa fa-sync fa-spin\'></i> '.__( "Verifying Request..." , "download-manager" ).'</div>").css("cursor","pointer").show().click(function(){ jQuery(this).hide();jQuery(".wpdmdlf_' . $field_id . '").show(); });
                if(res.downloadurl!=""&&res.downloadurl!=undefined) {
                window.open(res.downloadurl);
    ...
    public static function _Tweet($package, $buttononly = false)
    {
        ...
        $req = home_url('/?pid=' . $package['ID'] . '&var=' . $var);
        $home = home_url('/');
        $btitle = isset($package['tweet_heading']) ? $package['tweet_heading'] : __( "Tweet to download" , "download-manager" );
