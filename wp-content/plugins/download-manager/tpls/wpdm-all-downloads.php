    ...
    <div id="wpdm-all-packages">
        <table id="wpdmmydls-<?php echo $tid; ?>" class="table table-striped wpdm-all-packages-table">
            ...
            while ($q->have_posts()): $q->the_post();
                ...
                if(isset($data['base_price']) && $data['base_price'] > 0 && function_exists('wpdmpp_currency_sign')) $download_link = "<a href='#buy' data-package='{$data['ID']}' class='btn btn-sm1 btn-info wpdm-download-link wpdm-download-locked'>".__( "Buy" , "download-manager" )." ( ".$data['currency'].$data['effective_price']." )</a>";
