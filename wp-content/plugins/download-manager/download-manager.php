...
if(!isset($_SESSION) && !strstr($_SERVER['REQUEST_URI'], 'wpdm-media/') && !strstr($_SERVER['REQUEST_URI'], 'wpdm-download/') && !isset($_REQUEST['wpdmdl']))
    @session_start();

define('WPDM_Version','4.7.7');
...
class WordPressDownloadManager{
    ...
    function enqueueScripts()
    {
        ...
        if (!in_array('wpdm-bootstrap-css', $wpdmss))
            wp_enqueue_style('wpdm-bootstrap', plugins_url('/download-manager/assets/bootstrap'.$bsversion.'/css/bootstrap.min.css'));

        if (!in_array('wpdm-front', $wpdmss)) {
            wp_enqueue_style('wpdm-front', plugins_url() . '/download-manager/assets/css/front'.$bsversion.'.css', 9999999999);
        }

        if (!in_array('wpdm-bootstrap-js', $wpdmss))
            wp_enqueue_script('wpdm-bootstrap', plugins_url('/download-manager/assets/bootstrap'.$bsversion.'/js/bootstrap.min.js'), array('jquery'));
