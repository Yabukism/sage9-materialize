...
class WPEdenThemeEngine{
    ...
    function InitiateWidgets(){
        ...
        register_sidebar(array(
            ...
            'before_widget' => '<div class="widget panel panel-featured">',
            'after_widget' => '</div>',
    ...
    public static function FontCSS($typo){
        ...
        $fontsdata = WPEdenOptionFields::GetFontsJSON();
        $fonts = array();
        foreach($fontsdata as $font){
            $fonts[urlencode($font->family).":".implode(",", $font->variants)]['family'] = "\"{$font->family}\", {$font->category}";
        }
        ...
        switch($typo):
            case 'body_typo':
                if($body_font != '')
                    $font_family = isset($fonts[$body_font]) && $fonts[$body_font]['family'] != '' ? "font-family:{$fonts[$body_font]['family']} !important;" : "";
                else
                    $font_family = '';
            ...
            case 'typo_h3':
            case 'typo_h4':
                if($typo == 'typo_h2') $heading_font_size = intval( self::NextGetOption('heading2_font_size') );
                else if($typo == 'typo_h3') $heading_font_size = intval( self::NextGetOption('heading3_font_size') );
                else if($typo == 'typo_h4') $heading_font_size = intval( self::NextGetOption('heading4_font_size') );
                
                if($heading_font != '')
                    $font_family = isset($fonts[$heading_font]) && $fonts[$heading_font]['family'] != '' ? "font-family:{$fonts[$heading_font]['family']} !important;" : "";
                else
                    $font_family = '';
    ...
    function CustomCSS(){
        ...
        // Main Nav Colors ( From Customizer General Options )
        $main_navdd_bg = WPEdenThemeEngine::NextGetOption('main_navdd_bg', '');
        if($main_navdd_bg != '') $main_navdd_bg = "background: $main_navdd_bg;";
        $main_navdd_color = WPEdenThemeEngine::NextGetOption('main_navdd_color', '#333');
...
echo "<style>
$thenext_custom_css
body, p, .entry-content, .entry-content p{".self::FontCSS('body_typo')."}
    
h1{".self::FontCSS('typo_h1')."}
h2, h2 a{".self::FontCSS('typo_h2')."}
...
#mainframe.left-nav-layout, #header-2, #header-2 .navbar-default, #main-nav-container .navbar-eden{ $main_nav_bg }



@media (min-width: 768px) {
    #mainmenu .dropdown-menu { color: $main_navdd_color; text-shadow:none !important; $main_navdd_bg }
    #mainmenu>li>a { color: $main_nav_color; }
    #mainmenu .dropdown-menu *, #topnav-area .dropdown-menu a{  color: $main_navdd_color; }
